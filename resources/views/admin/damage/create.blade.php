                    ...
                    <div class="form-group">
                        ...
                        <select onchange="selectProduct(this)" class="form-control" name="select-product" id="select-product" required>
                            ...
                            @foreach ($products as $product )
                                <option value='{{$product->id}}' data-purchase-price="{{ $product->purchase_price ?? 0 }}" data-sell-price="{{ $product->sell_price ?? 0 }}" data-quantity="{{ $product->quantity ?? 0 }}">{{$product->name}}</option>
                            @endforeach
...
<script>
     ...
     function selectProduct($this){
           ...
           document.getElementById('stock_qnty').value = selectedOption.dataset.quantity || 0;
