                                    ...
                                    <td width="120px">
                                        <div class="input-group input-group-sm mb-1">
                                            ...
                                            <input type="text"
                                                   ...
                                                   onclick="console.log('Price input clicked')"
                                                   onblur="updateSellPrice({{ $item->id }}, this.value)">
                                        </div>
                                    </td>
                                    <td class="text-right">
...
function findOrderID() {
    ...
    $.ajax({
        url: '/admin/salesreturn/findorderid/' + orderId,
        method: 'GET',
...
function updateGrandTotals() {
    ...
    const grTotal = subTotal - discount;

    // Update display values
    $('#sub_total').text(subTotal.toFixed(2));
    $('#gr_total').text(grTotal.toFixed(2));

    // Update balance if customer is selected
    const customerId = $('#customer_id').val();
    if (customerId) {
        // Find customer balance from the customers array
        const customer = customers.find(c => c.id == customerId);
        if (customer) {
            const newBalance = customer.balance - grTotal;
            $('#new_balance').val(newBalance.toFixed(2));
            $('#last_balance').val(newBalance.toFixed(2));
        }
    }
...
function updateQuantity(productId, quantity) {
    ...
    $.ajax({
        url: '/admin/salesreturn-cart/change-qty',
        method: 'POST',
        data: {
        ...
        success: function(response) {
            console.log('Server response for quantity update:', response);
            ...
            var row = $('.product-row[data-product-id="' + productId + '"]');
            ...
            row.find('td:eq(3)').html('{{ config('settings.currency_symbol', '৳') }} ' + total);
            updateGrandTotals();
        },
        error: function(xhr) {
...
function updateSellPrice(productId, sellPrice) {
    console.log('updateSellPrice called with:', productId, sellPrice);
    if (!sellPrice || sellPrice <= 0) return;

    console.log('Sending sell price update:', { productId, sellPrice });
    $.ajax({
        url: '/admin/salesreturn-cart/change-sell-price',
        method: 'POST',
        data: {
            product_id: productId,
        ...
        success: function(response) {
            console.log('Server response for sell price update:', response);

            if (response.success === false) {
                Swal.fire('Error!', response.message || 'Failed to update price', 'error');
                return;
            }
        ...
        error: function(xhr) {
            const message = xhr.responseJSON?.message || 'Failed to update price';
            Swal.fire('Error!', message, 'error');
        }
    });
}
...
function removeFromCart(productId) {
    $.ajax({
        url: '/admin/salesreturn-cart/delete',
        method: 'POST',
        data: {
            product_id: productId,
            _method: 'DELETE',
...
function emptyCart() {
    $.ajax({
        url: '/admin/salesreturn-cart/empty',
        method: 'POST',
        data: {
            _method: 'DELETE',
            _token: '{{ csrf_token() }}'
...
function submitSalesReturn() {
    ...
    Swal.fire({
        ...
        preConfirm: (amount) => {
            return $.ajax({
                url: '/admin/salesreturn/finalsave',
                method: 'POST',
                data: {
                    customer_id: customerId,
